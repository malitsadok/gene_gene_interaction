...
subfolder_list=("${@:2}") 
csv_file="/sci/home/mali.tsadok/pipeline_scripts_singleton.csv" # Accepts additional arguments as the list of subfolders
...
for ((i = 0; i < ${#subfolders[@]}; i++)); do
  for ((j = i + 1; j < ${#subfolders[@]}; j++)); do
	...
	echo 'sbatch --array=1-$number  /sci/home/mali.tsadok/$script "$chr1" "$chr2" "$input_dir_full" "$output_dir_full"'
	
	sbatch --array=1-$number  /sci/home/mali.tsadok/singleton/$script "$chr1" "$chr2" "$input_dir_full" "$output_dir_full"
