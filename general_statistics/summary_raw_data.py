...
options_lof = [ 'frameshift_variant', 'frameshift_variant&splice_region_variant', 'frameshift_variant&start_lost',
...
options_missense = ["missense_variant", "missense_variant&splice_region_variant",
...
def pick_row(group):
    # LOF priority
    lof_rows = group[group["mutation_type"].isin(options_lof)]
    if not lof_rows.empty:
        return lof_rows.iloc[0]
    
    # Missense second priority
    missense_rows = group[group["mutation_type"].isin(options_missense)]
...
def process_joined_file(data_path, dict_path, join_on, num_genes):
    ...
    df = df[(df.mutation_type.isin(options_lof) )|  (df.mutation_type.isin(options_missense)) ]

    num_genes = df.gene.nunique() + num_genes
    
    # Flag mutation types (your existing logic)
    df['is_missense'] = df['mutation_type'].isin(options_missense)
    ...
    return summary_df , num_of_genes
...
def process_all_pairs(root_dir, join_on):
    ...
    for data_path in data_files:
        ...
        try:
            df_summary ,  num_of_genes= process_joined_file(data_path, dict_path, join_on , num_of_genes)
