...
def gene_gene_interactions_different_chromosomes (df1 ,df2, flag_same_file = False ) : 
    ...
    gene_gene_observed_df['both_lof'] = gene_gene_observed_df.apply(
    lambda row: calculate_cooccurrence(row, 'lof_1', 'lof_2'), axis=1)

    gene_gene_observed_df['both_missense'] = gene_gene_observed_df.apply(
    lambda row: calculate_cooccurrence(row, 'missense_1', 'missense_2'), axis=1)

    gene_gene_observed_df['both_lof_missense'] = gene_gene_observed_df.apply(
    lambda row: calculate_cooccurrence(row, 'lof_1', 'missense_2'), axis=1)

    gene_gene_observed_df['both_missense_lof'] = gene_gene_observed_df.apply(
    lambda row: calculate_cooccurrence(row, 'missense_1', 'lof_2'), axis=1)

    gene_gene_observed_df=gene_gene_observed_df.fillna(" ")
    gene_gene_grouped = gene_gene_observed_df.groupby(['gene_1', 'gene_2']).agg(
    both_lof=('both_lof', 'sum'),
    both_missense=('both_missense', 'sum'),
    both_lof_missense=('both_lof_missense', 'sum'),
    both_missense_lof=('both_missense_lof', 'sum'),
